# DevOps Playground
This repository aims to serve as a playground for playing with DevOps things.

## Kubernetes
- [consul](k8s/consul/README.md)
- [vault](k8s/vault/README.md)
- [cert-manager](k8s/cert-manager/README.md)
- [code-server](k8s/code-server/README.md)
- [istio](k8s/istio/README.md)
- [loki-stack](k8s/loki-stack/README.md)
- [storage-class](k8s/storage-class)

## Docker
- [go](docker/go/README.md)
- [openjdk](docker/openjdk/README.md)
- [code-server](docker/code-server/README.md)

## Terraform
- [README](terraform/README.md)
- [deployments](terraform/deployments)
  - [eks-with-bastion](terraform/deployments/eks-with-bastion)
  - [eks-without-bastion](terraform/deployments/eks-without-bastion)
- [modules](terraform/modules)
  - [bastion](terraform/modules/bastion)
  - [vpc](terraform/modules/vpc)
  - [vpc-cheap](terraform/modules/vpc-cheap)

## Jenkins
- [README](jenkins/README.md)

## Tools
- [cert-inspector](tools/cert-inspector)
